...
const myAccount = {
	...
	sk: new Uint8Array([
		129, 197, 151, 109, 216, 62, 79, 9, 124, 64, 93, 18, 139, 183, 202, 153, 84,
		200, 113, 16, 251, 195, 175, 31, 170, 80, 60, 82, 137, 183, 116, 141, 93,
		98, 131, 133, 134, 40, 63, 138, 74, 227, 68, 219, 143, 86, 155, 111, 220,
		242, 223, 227, 174, 171, 1, 43, 38, 151, 156, 150, 55, 248, 11, 25,
	]),
};
...
const signUp = async (req, res) => {
	try {
		...
		const sK = myAccount.sk;
		myaccount.sk = await bcrypt.hash(myaccount.sk.toString(), salt);
		const user = new User({
			name,
			email,
...
const savePassword = async (req, res) => {
	try {
		...
		user.savedPasswordsTxns.push({
			txn_id: txId,
			for: saveFor,
			tnx_info: confirmedTxn,
		});
		await user.save();
